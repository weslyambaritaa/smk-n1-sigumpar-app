...
export default function KebersihanKelas() {
    ...
    const { data, setData, post, processing, reset, errors } = useForm({
        tanggal: "", catatan: "", foto: null,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        post(route("wali-kelas.kebersihan.store"), { forceFormData: true, onSuccess: () => reset() });
    };
        ...
        <WaliKelasLayout>
            ...
            <div className="bg-white rounded-xl shadow p-6 max-w-2xl">
                ...
                <div className="mb-4">
                    ...
                    <textarea className="w-full border rounded px-3 py-2 text-sm resize-y" rows={5} placeholder="Catatan" value={data.catatan} onChange={(e) => setData("catatan", e.target.value)} />
                ...
                <div className="mb-6">
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center cursor-pointer hover:border-blue-400" onClick={() => fileRef.current?.click()}>
                        <p className="text-sm text-gray-500">{data.foto ? data.foto.name : "Upload Dokumentasi kebersihan"}</p>
                        <p className="text-xs text-gray-400 mt-1">Max. 5 MB</p>
                    </div>
                    <input ref={fileRef} type="file" className="hidden" accept=".jpg,.jpeg,.png" onChange={(e) => setData("foto", e.target.files[0])} />
                </div>
